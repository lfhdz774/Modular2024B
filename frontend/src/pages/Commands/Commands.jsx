...
function Command() {
  const [messages, setMessages] = useState([
    { text: 'Bienvenido al chat.', sender: 'sistema', type: 'text' },
  ]);
  const [newMessage, setNewMessage] = useState('');
  ...
  const handleSend = async () => {
    if (newMessage.trim() !== '') {
      ...
      setMessages((prevMessages) => [
        ...prevMessages,
        { text: newMessage, sender: 'yo', type: 'text' },
      ]);
      ...
      try {
        ...
        console.log(response);
        if (response.status === 200) {
          const responseData = response.data.respuesta;
          ...
          if (responseData.type === 'text') {
            ...
            // Mensaje complejo con múltiples partes
            const messageParts = responseData.message; // Esto es un array
              ...
              Object.entries(part).forEach(([key, value]) => {
                if (value.type === 'text') {
                  ...
                  newMessages.push({
                    ...
                    type: 'table',
                    title: key,
                    data: value.message, // Array de objetos
                  });
                }
              });
            });
          ...
          setMessages((prevMessages) => [
            ...prevMessages,
            { text: errorMessage, sender: 'sistema', type: 'text' },
          ]);
        }
      } catch (error) {
        setMessages((prevMessages) => [
        ...
        >
          {messages.map((message, index) => (
            ...
            >
              ...
              {/* Renderizar el mensaje según su tipo */}
              {message.type === 'text' && (
                <ListItemText
                  primary={message.text}
                  style={{
